   ...
   <div class="container">
         ...
         <table class="table custom-order">
            ...
            <tbody>
               ...
               <tr>
                  ...
                  <td><span class="total-price price-blue">%CURR_SIGN%%TOTAL_PRODUCT_PRICE%</span></td>
                  <td></td>
               </tr>
            </tbody>
         </table>
      ...
      <div class="form-group text-center" id="wallet">
         <form name="payWallet" method="post" action="">
            ...
            <input type="submit" class="btn btn-default blue-btn" type="submit" id="payNow" value="Pay" name="payWallet" title="Pay">
         </form>
      </div>
...
<script type="text/javascript">
   $(document).ready(function(){
       ...
       $(document).on("change","#deliveryOption",function(){
         var deliveryOption = $('#deliveryOption').val();
           ...
           if(deliveryOption != ''){
             $.ajax({
                   type: 'POST',
                   url: '<?php echo SITE_MOD; ?>orderCart-nct/ajax.orderCart-nct.php',
                   data: {deliveryOption:deliveryOption,action:'changeShipping'},
                   dataType : 'json',
         ...
         var pickId = $('#pickOption').val();
           if(pickId != ''){
             $.ajax({
                   type: 'POST',
                   url: '<?php echo SITE_MOD; ?>orderCart-nct/ajax.orderCart-nct.php',
                   data: {pickId:pickId,action:'changePickPoints'},
                   dataType : 'json',
                   success: function(data) {
       ...
       $(document).on("click","#coupon",function(){
         ...
         if(couponCode == ''){
           ...
           $.ajax({
               type: 'POST',
               url: '<?php echo SITE_MOD; ?>orderCart-nct/ajax.orderCart-nct.php',
               data: {couponCode:couponCode,action:'applyCoupon'},
               dataType : 'json',
               success: function(data) {
       ...
       $(document).on("change",".changeQuantity",function(){
         ...
         var quantity = $('#changeQuantity_'+cartId+'').val();
         
         window.location.href = '{SITE_URL}orderCart/changeQuantity/'+quantity+'/'+cartId
