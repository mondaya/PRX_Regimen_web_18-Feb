    ...
    $(function() {
	  	OTable = $('#example123').dataTable( {
			...
			"fnServerData": function (sSource, aoData, fnCallback) {
			 ...
			 "aoColumns": [
				{ "sName": "storeName", 'sTitle' : 'Store Name'},
				{ "sName": "storeImage", 'sTitle' : 'Store Image', bSearchable:false, bSortable: false}
		...
		$(document).on('submit','#frmCont', function(e){
			$("#frmCont").validate({
				...
				rules: {
					...
					storeImage: {extension: 'jpg|jpeg|png'}
				},
				messages:{
					storeName: { required: 'Please enter store name' },
					storeLink: { required: 'Please enter store link' },
					'categoryId[]': { required: "&nbsp;Please select category"},
					'subcategoryId[]': { required: "&nbsp;Please select sub category"},
					storeImage: {extension: 'Only jpeg and png image type are supported.'}
				},
