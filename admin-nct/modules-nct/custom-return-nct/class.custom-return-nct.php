<?php
class custom_return_request extends Home {
	...
	public function __construct($module, $id=0, $objPost=NULL, $searchArray=array(), $type='') {
		...
		$this->fields = $fields;
		$this->module = $module;
		$this->table = 'tbl_custom_return';

		$this->type = ($this->id > 0 ? 'edit' : 'add');
		$this->searchArray = $searchArray;
		parent::__construct();
		if($this->id>0){
			$qrySel = $this->db->pdoQuery("SELECT r.id,r.subject,r.message,u.firstName,u.lastName,u.email,u.paypalEmail,o.productName,r.subject,r.adminPaid,o.paidAmount,o.orderId,o.quantity,o.createdDate,o.discountAmount,o.deliveryOption,o.dutiesAmount,o.adminCharge,o.shippingAmount,o.paidAmount,o.deliveryStatus,o.productPrice,ct.cityName,co.countryName,s.stateName 
				  FROM tbl_custom_return as r
				  ...
				  LEFT JOIN tbl_custom_orders as o ON(r.orderId = o.id) where r.id = ".$this->id."")->result();
	...
	public function viewForm(){
		...
		//For return image
		$addedImage = '';
		$query = $this->db->pdoQuery("select id,imageName from tbl_customreturn_image where returnId = ".$this->id."")->results();
	...
	public function dataGrid() {
		...
		$sWhere = ' WHERE 1 = ? ';
		if(isset($chr) && $chr != '') {
			$sWhere .= " AND (o.productName LIKE ? OR u.firstName LIKE ? )";
			$aWhere[] = "%$chr%";
			$aWhere[] = "%$chr%";
		}
		...
		$qrySel1 = $this->db->pdoQuery("SELECT r.id FROM tbl_custom_return as r
		 LEFT JOIN tbl_users as u ON(r.userId = u.id)
		 LEFT JOIN tbl_custom_orders as o ON(r.orderId = o.id)
		 $sWhere ".$whereCond." ORDER BY $sorting", $aWhere)->results();

		$qrySel = $this->db->pdoQuery("SELECT r.id,u.firstName,o.productName,r.subject,r.adminPaid,o.paidAmount FROM tbl_custom_return as r
		 LEFT JOIN tbl_users as u ON(r.userId = u.id)
		 LEFT JOIN tbl_custom_orders as o ON(r.orderId = o.id)
		...
		foreach($qrySel as $fetchRes) {
			...
			if($fetchRes['adminPaid'] == 'n'){
				$operation .= (in_array('pay',$this->Permission))?'&nbsp;&nbsp;'.$this->operation(array("href"=>SITE_URL.'admin-nct/modules-nct/custom-return-nct/index.php?action=pay&id='.$id.'',"class"=>"btn default blue btn-xs","value"=>'&nbsp;Pay')):'';
			}
