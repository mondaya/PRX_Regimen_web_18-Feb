<?php
	...
	if($action == "updateStatus") {
		...
		add_admin_activity($activity_array);
	}else if ($action == 'changeState' && $action !='') {
		$content = '';
		$countryId = $_REQUEST['country'];

		$qrySel=$db->pdoQuery("SELECT id,stateName from tbl_state where isActive = 'y' and countryId = ".$countryId."");
		$fetchRes = $qrySel->results();
		$totalRow = $qrySel->affectedRows();	

		$content = '<option name="state" id="state" value="">Select state</option>';	
				        if($totalRow > 0){ 
							foreach($fetchRes as $fetchRes){
								$stateId = $fetchRes['id'];
								$stateName = $fetchRes['stateName'];
									
								$content .= '<option value="'.$stateId.'">'.$stateName.'</option>';
							}
						}
	}

	$mainObject = new shipping($module, $id, NULL, $searchArray, $action);
