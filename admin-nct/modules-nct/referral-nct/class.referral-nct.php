...
class referral extends Home {
	...
	public function __construct($module, $id=0, $objPost=NULL, $searchArray=array(), $type='') {		
		...
		parent::__construct();	
		if($this->id>0){
			$qrySel = $this->db->select($this->table, array("id","referral"),array("id"=>$id))->result();
			$fetchRes = $qrySel;
			$this->data['referral'] = $this->referral = $fetchRes['referral'];
		}else{
			$this->data['referral'] = $this->referral = '';
		}
		switch($type){
			case 'add' : {
	...
	public function viewForm(){		
		$content = $this->displayBox(array("label"=>"Referral Amount (%)&nbsp;:","value"=>$this->referral));
		return $content;
	}
	public function getForm() {
		...
		$content = '';
			...
			$fields = array("%MEND_SIGN%","%REFERRAL%","%TYPE%","%ID%");
	
			$fields_replace = array(MEND_SIGN,$this->data['referral'],$this->type,$this->id);
	...
	public function dataGrid() {
		...
		$qrySel = $this->db->select("tbl_admin_amount",array("id","referral"), $whereCond, " ORDER BY $sorting limit $offset , $rows" )->results();
		foreach($qrySel as $fetchRes) {
			...
			$final_array =  array($fetchRes["referral"]);
