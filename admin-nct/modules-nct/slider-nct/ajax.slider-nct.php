...
//echo $_REQUEST['pageName'];exit;
	...
	if(isset($_POST["ajaxvalidate"]) && $_POST["ajaxvalidate"]==true) {
	...
	else if($action == "delete_images_direct") {
		
	    $_SESSION['temp_files'][$image_id]='';
	    echo json_encode(array('data'=>'done'));
	}  
		...
		$qrySel = $db->pdoQuery("SELECT * from tbl_slider WHERE id=".$id)->result();
		$fetchRes = $qrySel;
		$old_image =$fetchRes['sliderImage'];
			
		$aWhere=array("id"=>$id);
		$db->delete($table,$aWhere);
		@unlink("../../../upload-nct/sliderImage/".$old_image."");
