<?php
	...
	if(isset($_POST["submitAddForm"]) && $_SERVER["REQUEST_METHOD"] == "POST") {
		...
		$objPost->CountryID = isset($CountryID) ? $CountryID : '';
		$objPost->isActive	= isset($isActive) && $isActive == 'y' ? 'y' : 'n';
	
		if($objPost->stateName != ""){
			if($type == 'edit' && $id > 0){
				if(in_array('edit',$Permission)){ 	
					$db->update($table, array('stateName'=>$objPost->stateName,"CountryID"=>$objPost->CountryID,'isActive'=>$objPost->isActive), array("id"=>$id));
				...
				if(in_array('add',$Permission)){
					...
					if(getTotalRows($table,array('stateName'=>$objPost->stateName))==0){
	...
	$objState = new State($module, $id, NULL, $searchArray, $type);
