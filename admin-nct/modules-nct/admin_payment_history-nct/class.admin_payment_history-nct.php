<?php
class AdminPaymentHistory extends Home {
	public $data = array();
	...
	public function __construct($module, $id=0, $objPost=NULL, $searchArray=array(), $type='') {
		...
		$this->fields = $fields;
		$this->module = $module;
		$this->table = 'tbl_admin_payment_history';
	...
	public function dataGrid() {
		...
		if(isset($chr) && !empty($chr)) {
			$whereCond = " AND (u.firstName LIKE ?)";
			$aWhere[] = "%$chr%";
		}
		...
		if(!empty($date_range) && $date_range!='~'){
			...
			if(!empty($date_range[0]) && !empty($date_range[1])) {
				...
				$whereCond .= " AND ph.createdDate >= ?  AND ph.createdDate <= ?";
		...
		if(isset($sort)){
			...
			if($sort == 'firstName'){
				$alias = 'u.';
			}else if($sort == 'amount' || $sort == 'transactionType' || $sort == 'createdDate'){
				$alias = 'ph.';
			}
		...
		$qrySel1 =$this->db->pdoQuery("SELECT ph.id FROM $this->table as ph INNER JOIN tbl_users as u ON u.id=ph.userId".$whereCond." ORDER BY ".$sorting."", $aWhere)->results();
		
		$totalRow = count($qrySel1);
		$qrySel =$this->db->pdoQuery("SELECT ph.*, u.firstName FROM $this->table as ph INNER JOIN tbl_users as u ON u.id=ph.userId".$whereCond." ORDER BY ".$sorting." LIMIT ".$offset." ,".$rows." ", $aWhere)->results();


		foreach($qrySel as $fetchRes) {
			$paymentType = $fetchRes['transactionType']=='a'?'Added':'Removed';
			...
			$final_array =  array($fetchRes["firstName"], (SITE_CURR.number_format(($fetchRes["amount"]), 2)),$paymentType, date('m-d-Y', strtotime($fetchRes["createdDate"])));
