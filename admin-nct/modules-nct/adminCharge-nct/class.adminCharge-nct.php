<?php
class adminCharge extends Home {
	...
	public function __construct($module, $id=0, $objPost=NULL, $searchArray=array(), $type='') {
		...
		$this->fields = $fields;
		$this->module = $module;
		$this->table = 'tbl_admin_charge';

		$this->type = ($this->id > 0 ? 'edit' : 'add');
		$this->searchArray = $searchArray;
		parent::__construct();
		if($this->id>0){
			$qrySel = $this->db->pdoQuery("SELECT da.id,da.countryId,da.amount,da.minimumAmount,da.isActive,c.countryName from tbl_admin_charge as da LEFT JOIN tbl_country as c ON(da.countryId = c.id) where da.id = ".$this->id."")->result();
	...
	public function viewForm(){
		...
		$this->displayBox(array("label"=>"Admin Charge Amount(%)&nbsp;:","value"=>$this->amount)).
		$this->displayBox(array("label"=>"Minimum Admin Charge Amount&nbsp;:","value"=>SITE_CURR.$this->minimumAmount)).
	...
	public function getForm() {
		...
		$fields = array("%MEND_SIGN%","%COUNTRY_OPTION%","%ADMIN_CHARGE%","%STATUS_A%","%STATUS_D%","%TYPE%","%ID%","%CAT_PHOTO%","%ADD_MORE%","%MINIMUM_AMOUNT%","%SITE_CURR%");
	...
	public function dataGrid() {
		...
		$qrySel1 = $this->db->pdoQuery("SELECT da.id from tbl_admin_charge as da LEFT JOIN tbl_country as c ON(da.countryId = c.id)
			$sWhere ORDER BY $sorting", $aWhere)->results();

		$qrySel = $this->db->pdoQuery("SELECT da.id,da.amount,da.minimumAmount,da.isActive,c.countryName from tbl_admin_charge as da LEFT JOIN tbl_country as c ON(da.countryId = c.id)
